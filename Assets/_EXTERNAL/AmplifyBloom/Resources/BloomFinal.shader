...
Shader "Hidden/BloomFinal"
{
	...
	CGINCLUDE
		...
		inline half4 FinalComposition( v2f_img_custom input, const int count, const bool flare, const bool glare, const bool dirt, const bool starburst )
		{
			...
			b0 =  DecodeColor ( tex2D ( _MipResultsRTS0, input.uv ) );
			if ( count > 1 ) b1 = DecodeColor ( tex2D ( _MipResultsRTS1, input.uv ) );
			if ( count > 2 ) b2 = DecodeColor ( tex2D ( _MipResultsRTS2, input.uv ) );
			if ( count > 3 ) b3 = DecodeColor ( tex2D ( _MipResultsRTS3, input.uv ) );
			if ( count > 4 ) b4 = DecodeColor ( tex2D ( _MipResultsRTS4, input.uv ) );
			if ( count > 5 ) b5 = DecodeColor ( tex2D ( _MipResultsRTS5, input.uv ) );
			...
			if ( flare )
			{
				half4 lensFlareColor = tex2D ( _LensFlare, input.uv );
			...
			if ( glare )
			{
				half4 lensGlareColor = tex2D ( _LensGlare, input.uv );
